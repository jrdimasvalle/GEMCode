...
h1 = setEffHisto2("h_eta_me1_after_lct_okAlctClct","h_eta_me1_initial",d1, etareb, ROOT.kRed, 1, 2, ROOT.kBlue, 4, topTitle,xTitle,yTitle,xrange,yrange)
...
b1.GetYaxis().SetRangeUser(1800,2120)
...
for x in range(xmin, xmax):
    ...
    jkl = e2.GetBinContent(e2.FindBin(s))
    ppp = h1.GetBinContent(h1.FindBin(s))
    diffa=jkl-ppp
    print "Eta: %10s"%s,"    Tree: %7s"%jkl, "    Histogram: %7s"%ppp, "    Tree - Hist: %5s"%diffa
    mlo=mlo+jkl
    pqr=pqr+ppp
    pta=pta+abs(diffa)
