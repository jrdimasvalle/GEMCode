...
std::ostream & operator<<(std::ostream & o, const matching::Digi& d)
{
  ...
  if (t==CSC_CLCT or t == CSC_ALCT or t == CSC_LCT) {

    CSCDetId idma(id);
    if (t == CSC_CLCT) o<<id<<" <=> "<<idma<<" t: CLCT hs "<<ch<<" bx "<<bx<<" q "<<q<<" p "<<p;
    else if (t == CSC_ALCT) o<<id<<" <=> "<<idma<<" t: ALCT wg "<<ch<<" bx "<<bx<<" q "<<q;
    else if (t == CSC_LCT) o<<id<<" <=> "<<idma<<" t: LCT hs "<<ch<<" wg "<<wg<<" bx "<<bx<<" q "<<q<<" p "<<p<<" dphi "<<dphi;
    else o<<id<<" t: "<<t<<" ch "<<ch<<" bx "<<bx;

  }else{ 

    if (t == GEM_PAD) o<<id<<" t: GEM Pad  pad "<<ch <<" bx "<<bx;
