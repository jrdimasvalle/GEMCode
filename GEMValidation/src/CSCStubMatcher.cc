...
// for The moment I'll work only in CSC
/*
...
//Those other variables will work on non CSC only
...
// Redefined Variables because of the vector nature
    ...
    for (auto &lct: lcts_tmp)
    {
        ...
        for(unsigned int i=0;i<clct.size(); i++ ) {
            ...
            for (unsigned int j=0; j<alct.size();j++){
                ...
                std::cout<<" LCT info: BX "<<digi_bx(lct)<<" HS: "<<digi_channel(lct)<<" WG: "<<digi_wg(lct)<<std::endl;
              }
    
        } // Added for

      std::cout<<" ------------------------------------- Ending Matching -------------------------------------------- "<<std::endl;

       if (foundit_match==0) continue;

        std::cout<<" If there is NOT a match this shouldn't be printed"<<std::endl;
    // No GEM or RPC for the moment
    /*
      ...
      if (matchClctRpc_ and caseClctRpc and !(my_bx == digi_bx(lct) and my_hs == digi_channel(lct) and std::abs(my_wg_gemrpc_mean - digi_wg(lct))<3) ){
        ...
        */
        ...
        std::cout<<" LCT passed: BX: "<<digi_bx(lct)<<" HS: "<<digi_channel(lct)<<" WG "<<digi_wg(lct)<<std::endl;
