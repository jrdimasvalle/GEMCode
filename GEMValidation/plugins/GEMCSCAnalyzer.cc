...
void MyTrackEffDT::init()
{
 ...
 phi_gv_rpc=-9999.;
 eta_gv_rpc=-9999.;
 deltaphi_dt_rpc_gp = - 9999.;
 deltaphi_dt_rpc_gv=-9999.;
...
TTree*MyTrackEffDT::book(TTree *t,const std::string & name)
{
  ...
  t->Branch("has_rpc_sh_dt", &has_rpc_sh_dt);
  t->Branch("x_gp_rpc", &x_gp_rpc);
  t->Branch("y_gp_rpc", &y_gp_rpc);
  t->Branch("z_gp_rpc", &z_gp_rpc);
  t->Branch("r_gp_rpc", &r_gp_rpc);
  t->Branch("eta_gp_rpc", &eta_gp_rpc);
  t->Branch("phi_gp_rpc", &phi_gp_rpc);
  t->Branch("pt_gv_rpc", &pt_gv_rpc);
  t->Branch("eta_gv_rpc", &eta_gv_rpc);
  t->Branch("phi_gv_rpc", &phi_gv_rpc);
  t->Branch("deltaphi_dt_rpc_gv", &deltaphi_dt_rpc_gv);
  t->Branch("deltaphi_dt_rpc_gp", &deltaphi_dt_rpc_gp);
...
void GEMCSCAnalyzer::analyzeTrackEff(SimTrackMatchManager& match, int trk_no)
{
  ...
  for (auto ddt: dt_simhits)
  {
    ...
    for (auto drp:rpc_ch_ids) //Looking for a second hit in RPC
    {
      RPCDetId idrp(drp);
      ...
      GlobalVector ym_rpc = match_sh.detDTRPCGlobalPT(match_sh.hitsInChamber(drp));
      etrk_dt_[stdt].has_rpc_sh_dt = 1;
      etrk_dt_[stdt].x_gp_rpc = hitGp_rpc.x();
      etrk_dt_[stdt].y_gp_rpc = hitGp_rpc.y();
      etrk_dt_[stdt].z_gp_rpc = hitGp_rpc.z();
      etrk_dt_[stdt].r_gp_rpc = hitGp_rpc.perp();
      etrk_dt_[stdt].eta_gp_rpc = hitGp_rpc.eta();
      etrk_dt_[stdt].phi_gp_rpc = hitGp_rpc.phi();
      etrk_dt_[stdt].pt_gv_rpc = ym_rpc.perp();
      etrk_dt_[stdt].phi_gv_rpc = ym_rpc.phi();
      etrk_dt_[stdt].eta_gv_rpc = ym_rpc.eta();
      etrk_dt_[stdt].deltaphi_dt_rpc_gv = -ym_rpc.phi() + ym.phi();
    ...
    for (auto s_ddt: dt_simhits) // Looking for a second hit in DT stations 
    {
     ...
     }   // End of second station on DT SimHit.
